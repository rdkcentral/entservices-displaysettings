...
- **REQ-DS-015**: `getSupportedResolutions` SHALL return the list of resolutions sourced from the connected display's EDID data/Platform capabilities, obtained via the `getSupportedTvResolutions` HAL API.
...
- **REQ-DS-018**: `getSupportedResolutions` SHALL reflect the newly connected display's EDID/platform capabilities after a display hot-plug event; stale data from the previous display SHALL NOT be returned.

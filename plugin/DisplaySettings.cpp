...
//            if (((m_hdmiInAudioDevicePowerState == AUDIO_DEVICE_POWER_STATE_ON) || (retryPowerRequestCount >= 5)) && m_AudioDevicePowerOnStatusTimer.isActive()) {
    ...
    Core::hresult DisplaySettings::Request(const string& newState,const JsonObject& response)
	{
        ...
        void DisplaySettings::OnAudioFaderControlChanged(int mixerBalance,int a,int c)
        {
            LOGINFO("Received OnAudioFaderControlChanged. Fader Control: %d,%d,%d", mixerBalance,a,c);
            if(DisplaySettings::_instance)
            {
